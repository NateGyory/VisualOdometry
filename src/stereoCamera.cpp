...
void InitStereoCamera(StereoCamera &stereo_cam)
{
    ...
    cv::stereoRectify(stereo_cam.L_cam.K, stereo_cam.L_cam.D, stereo_cam.R_cam.K, stereo_cam.R_cam.D, stereo_cam.imageSize, T_R_L.rowRange(0,3).colRange(0,3), T_R_L.rowRange(0,3).col(3), R1, R2, P1, P2, stereo_cam.Q);

    stereo_cam.L_cam.P = P1;
    stereo_cam.R_cam.P = P2;

    cv::initUndistortRectifyMap(stereo_cam.L_cam.K, stereo_cam.L_cam.D, R1,
                                P1.rowRange(0,3).colRange(0,3), stereo_cam.imageSize,
                                CV_32FC1, stereo_cam.L_cam.map1, stereo_cam.L_cam.map2);

    cv::initUndistortRectifyMap(stereo_cam.R_cam.K, stereo_cam.R_cam.D, R2,
                                P2.rowRange(0,3).colRange(0,3), stereo_cam.imageSize,
